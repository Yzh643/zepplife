...
class ZeppStepSetter:
    ...
    def login(self):
        ...
        try:
            ...
            return "access_code 获取失败"

        print("access_code 获取成功")
...
def on_submit():
    ...
    setter = ZeppStepSetter(USER, PASSWORD, STEP)
    response = setter.main()

    # 处理返回的响应结果并显示
    if isinstance(response, dict):
        result_msg = f"{response.get('message', '无返回消息')}，修改步数为: {STEP}"
    else:
        result_msg = response  # 如果是登录失败或其他错误信息
...
def open_link(event):
    webbrowser.open_new("https://docs.qq.com/document/DWW1oc3hkY3BKWGZR")

link_label = tk.Label(root, text="使用教程", fg="blue", cursor="hand2")
link_label.pack(pady=(10, 0))  # 顶部留一点间距
link_label.bind("<Button-1>", open_link)

# 创建输入框和标签
tk.Label(root, text="账户邮箱/手机号").pack(pady=5)
...
tk.Label(root, text="目标步数").pack(pady=5)
entry_step = tk.Entry(root, width=30)
